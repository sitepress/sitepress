...
module Sitepress
  ...
  module SitePages
    ...
    # Determine if the `view_name` being passed is a string of a constantizable
    # Phlex component / layout.
    def phlex_component?(view_name)
      return false if view_name.instance_of?(ActionView::Template)
    ...
    def controller_layout(resource)
      ...
      if phlex_component?(layout) # We have a constant passed that is a Phlex component
        proc { layout }
      elsif layout.instance_of? String # Rails 4 and 5 return a string from above.
        layout
